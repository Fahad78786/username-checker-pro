...
router.post('/signup', async (req, res) => {
  try {
    ...
    if (existErr) return res.json({ success: false, message: 'DB Error: ' + existErr.message });
    ...
    const { data: user, error } = await supabase.from('users').insert([{
      ...
      points: 0, is_blocked: false,
      country, country_code: countryCode, ip, total_checked: 0
    }]).select().single();

    if (error) return res.json({ success: false, message: 'Insert Error: ' + error.message });
    ...
    res.json({
      ...
      token,
      user: { id: user.id, username: user.username, email: user.email, points: user.points, country: user.country }
    });
  } catch (err) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    const { data: users, error } = await supabase.from('users').select('*')
      .or(`email.eq.${email},username.eq.${email}`).limit(1);
    ...
    const ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress || '';
    const { country, countryCode } = await getCountry(ip);
    await supabase.from('users').update({ last_login: new Date(), ip, country, country_code: countryCode }).eq('id', user.id);

    const token = jwt.sign({ id: user.id, isAdmin: false }, process.env.JWT_SECRET, { expiresIn: '7d' });
    res.json({
      success: true, isAdmin: false, token,
      user: { id: user.id, username: user.username, email: user.email, points: user.points, country: user.country }
    });
  } catch (err) {
    res.json({ success: false, message: 'Server error: ' + err.message });
